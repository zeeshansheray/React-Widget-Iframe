...
export default function CustomDropDown({width, className, showCheckBox, classNameB, search, options, addNewProduct, sufix, placeholderImage, label, allFilterShow, setFilterData}) {
  ...
  return (
    <div>
        ...
        <div className = {'customDropDownClosed wpx-' + width + ' '+ className} onClick = {()=>{setShow(!show);}}>
        ...
        {show && <div className={`customDropDown wpx-${width} ${classNameB?classNameB:className}`}>
            ...
            <div className='options'>
                    ...
                    <CustomCheckBox 
                                    value={allFilter} onChange={(e)=>{
                                        if(!allFilter) {
                                            ...
                                            // options && options.length > 0 && options.map((element, idx)=>{
                                            //     data.push({...element, checkBox: true})
                                            // })
                ...
                }
                {
                    filteredOptions && filteredOptions.length > 0 && filteredOptions.map((option, idx)=>(
                        ...
                        onClick={()=>{setSelectedLabel(option.label); setFilterData([option.value]); if(!showCheckBox) setShow(!show)}}
                        >
                            {
